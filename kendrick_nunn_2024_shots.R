...
df <- read_csv("kendrick_nunn_2024_shots.csv")
...
# 3-point arc (ONLY the curved arc — no corner lines)
arc3 <- data.frame(
  x = rim_x + 675 * sin(seq(-pi/2, pi/2, length.out = 300)),
  y = rim_y + 675 * cos(seq(-pi/2, pi/2, length.out = 300))
)

arc3_full <- arc3   # ← remove corner vertical lines
...
shot_chart <- ggplot() +
  ...
  geom_point(data = shots_made,
             ...
             colour = "red", alpha = 0.8, size = 3) +
  coord_fixed() +
  theme_minimal() +
  theme(panel.grid = element_blank()) +
  labs(
    title = "Kendrick Nunn — Made Shot Locations (EuroLeague 2024)",
